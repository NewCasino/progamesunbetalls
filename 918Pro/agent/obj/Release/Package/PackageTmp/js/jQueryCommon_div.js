...
function ShowTipDiv(timeout) {
    ...
    if ($("#datagrid-mask").length == 0) {
        $("<div id=\"datagrid-mask\" style=\"background:#666666;position:absolute;left:0;top:0; width:100%\"></div>").css({ display: "none",  height: $("body").height() }).appendTo("body");
        $("<div id=\"datagrid-mask-msg\" style=\"position:absolute;\"></div>").html("<img src='/images/loading.gif'/>").appendTo("body").css({ display: "none", left: ($(window).width() - 16) / 2 + 'px', top: ($(window).height() - 16) / 2 + 'px' });
    }
