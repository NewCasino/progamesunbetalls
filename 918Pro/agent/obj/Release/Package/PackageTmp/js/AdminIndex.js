...
function InitSideBarState() {
    ...
    if (existentSideBarCookie.length != 0 && SideBarKey.length != 0 && existentSideBarCookie.indexOf(SideBarKey) != -1) {
        ...
        obj.title = languages.关闭左栏;
        obj.className = "btn_close";
...
function ChangeSideBarState(temp) {
    var obj = document.getElementById("switchPoint");
    if (temp == "none") {
        obj.className = "btn_open";
        obj.title = languages.打开左栏;
        obj.onmouseover = function(){this.className='btn_open_over'};
        obj.onmouseout =  function(){this.className='btn_open'};
       // obj.src = "/images/default/butOpen.gif";
        ...
        obj.className = "btn_close";
        obj.title = languages.关闭左栏;
        obj.onmouseover =  function(){this.className='btn_close_over'};
        obj.onmouseout =  function(){this.className='btn_close'};
       // obj.src = "/images/default/butClose.gif";
        document.getElementById("frmTitle").style.display = "block";
        onload();
    }
}
...
function switchSysBar() {
    var obj = document.getElementById("switchPoint");
    if (obj.className == "btn_close_over") {
        obj.className = "btn_open";
        obj.title = languages.打开左栏;
        obj.onmouseover = function(){this.className='btn_open_over'};
        obj.onmouseout =  function(){this.className='btn_open'};
       // obj.src = "/images/default/butOpen.gif";
        ...
        height = document.body.clientHeight - 121;
        document.getElementById("main_right").style.height = height;
        document.getElementById("main_right").style.width = width;
        //document.getElementById("FrameTabs").style.width = width;
        if (CheckFramesScroll) { CheckFramesScroll(); }
...
function ShowHideLayer(ID) {
    if (ID != tID) {
        ...
        if (tID != "") {
            document.getElementById("A" + tID).style.backgroundImage = "url(/images/default/digital_left.gif)";
            if (document.getElementById("A" + tID).childNodes.length < 2) {
                document.getElementById("Span" + tID).style.backgroundImage = "url(/images/default/digital_side.gif)";
            } else {
        ...
        document.getElementById("A" + ID).style.backgroundImage = "url(/images/default/seg_left.gif)";
        if (document.getElementById("A" + ID).childNodes.length < 2) {
            document.getElementById("Span" + ID).style.backgroundImage = "url(/images/default/seg_side.gif)";
            if (tID != "" && document.getElementById("A" + tID).childNodes.length >= 2) {
                if (triangle) { triangle.style.backgroundImage = "url(/images/default/digital_side.gif)"; }
            }
        } else {
            document.getElementById("Span" + ID).style.backgroundImage = "none";
            if (triangle) { triangle.style.backgroundImage = "url(/images/default/seg_side.gif)"; }
        }
