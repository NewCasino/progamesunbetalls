...
namespace DAL
{
    ...
    {
        private const string SQL_INSERT = "insert into yafa.sys_module (Module_parent_code,Module_text,Module_url,Module_target,status,sorts,BetGamesID,Module_type,Module_tip)values(?Module_parent_code,?Module_text,?Module_url,?Module_target,?status,?sorts,?BetGamesID,?Module_type,?Module_tip)";
        private const string SQL_UPDATE = "update yafa.sys_module set Module_parent_code=?Module_parent_code,Module_text=?Module_text,Module_url=?Module_url,Module_target=?Module_target,status=?status,sorts=?sorts,BetGamesID=?BetGamesID,Module_type=?Module_type,Module_tip=?Module_tip where Module_code = ?Module_code";
        private const string SQL_SELECTBYPK = "select Module_code from yafa.sys_module  where sys_module.Module_code = ?Module_code";
        private const string SQL_SELECTALL = "select Module_code,Module_parent_code,Module_text,Module_url,Module_target,status,sorts,BetGamesID,Module_type,Module_tip from yafa.sys_module ";
        private const string SQL_DELETEBYPK = "delete  from yafa.sys_module  where sys_module.Module_code = ?Module_code";

        private const String SELECT_MODULE_ALL = "select m.Module_code,m.Module_parent_code,m.Module_type,m.Module_text,m.Module_url,m.Module_target,m.Module_tip,m.status from sys_module m  ";

        private const String UPDATE_MODULE_STATUS = "update sys_module set status=?status where Module_code like ?Module_code ";
        ...
        private const String SELECT_LASTMODULE_BYPARENTMODULE = "select m.Module_code from sys_module m where m.Module_parent_code=?moduleParentCode " + ORDERBY + " desc limit 1";

        private const String INSERT_MODULE = "insert into sys_module (Module_code,Module_parent_code,Module_type,Module_text,Module_url,Module_target,Module_tip,status) values(?ModuleCode,?ModuleParentCode,?ModuleType,?ModuleText,?ModuleUrl,?ModuleTarget,?ModuleTip,?status);";

        private const String UPDATE_PARENT_MODULE = "update sys_module set module_text=?moduleText,module_tip=?moduleTip where module_code=?moduleCode";

        private const String UPDATE_CHILD_MODULE = "update sys_module set module_text=?moduleText,module_tip=?moduleTip,module_url=?moduleUrl,module_target=?moduleTarget where module_code=?moduleCode";

        private const String SELECT_MODULE_STATUS = SELECT_MODULE_ALL + " where m.status=?status order by m.Module_code";

        private const String SELECT_BYPID = "select m.* from sys_module m where m.Module_parent_code=?moduleParentCode";

        private Sys_module_rightService sysModuleRightService = new Sys_module_rightService();
        ...
        /// </summary>
        /// <returns></returns>
        public IList<Sys_module> GetRootModule()
        {
        ...
        public IList<Sys_module> GetSysModules()
        {
            return GetAllModuleBySql(SELECT_MODULE_ALL + ORDERBY, null);
        }

        public IList<Sys_module> GetSysModuleByStatus(string status)
        {
            MySqlParameter[] param = new MySqlParameter[]{
                new MySqlParameter("?status",status)
            };
        ...
        ///生成时间：2010-9-18 13:49:34		
        ///</summary>		
        public Boolean UpdateSys_module(Sys_module sys_module)
        {
            MySqlParameter[] param = new MySqlParameter[]{
				 new MySqlParameter("?Module_parent_code",sys_module.Module_parent_code),
				 new MySqlParameter("?Module_text",sys_module.Module_text),
				 ...
				 new MySqlParameter("?Module_type",sys_module.Module_type),
				 new MySqlParameter("?Module_tip",sys_module.Module_tip),
				 new MySqlParameter("?Module_code",sys_module.Module_code)
			};
        ...
        ///生成时间：2010-9-18 13:49:34		
        ///</summary>		
        public Boolean DeleteSys_moduleByPK(object id)
        {
            MySqlParameter[] param = new MySqlParameter[]{
        ...
        ///生成时间：2010-9-18 13:49:34		
        ///</summary>		
        public Sys_module GetSys_moduleByPK(object id)
        {
            MySqlParameter[] param = new MySqlParameter[]{
				 new MySqlParameter("?Module_code",id)
			};

            return MySqlModelHelper<Sys_module>.GetSingleObjectBySql(SQL_SELECTBYPK, param);
        }
        ...
        ///生成时间：2010-9-18 13:49:34		
        ///</summary>		
        public IList<Sys_module> GetMutilILSys_module()
        {
            return MySqlModelHelper<Sys_module>.GetObjectsBySql(SQL_SELECTALL, null);
        }
        ...
        ///生成时间：2010-9-18 13:49:34		
        ///</summary>		
        public DataTable GetMutilDTSys_module()
        {
