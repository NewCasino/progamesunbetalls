(function ($) {
    ...
    function checkUser(element) {
        ...
        var $user = element;
        var u_huiyuan = '718s' + $user.val();


        if (u_huiyuan.length < 6 || u_huiyuan.length > 12) {

            $('#TipData').html("用户名称不合法!<br/>请重新选择!(账户由718s加2-6位字母，数字组成（禁止汉字）");
            $("#dialog2").dialog("open");
            ...
            jQuery.AjaxCommon({
                ...
                toSuccess: function (json) {
                    if (json.d) {
                        ...
                        $('#TipData').html("<i></i>号码已存在,请使用新的号码注册<br/>请重新选择!(账户由718s加2-6位字母，数字组成（禁止汉字）");
                        $("#dialog2").dialog("open");
                        $("#userb").focus();
                        flagOBJ.userName_F = false;
                        return false;
    ...
    $("#btnSubmit").click(function () {
        ...
        var usernames = "718s" + $("#userb").val().replaceDanYinHao();
