<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="percent.aspx.cs" Inherits="admin.User.percent" %>
        ...
        function setDate(data) {
            ...
            jQuery.AjaxCommon("/ServicesFile/UserService.asmx/GetAgents", data, true, false, function (json) {
                if (json.d != "none") {
                    ...
                    var result = jQuery.parseJSON(json.d);
                    if (pd) {
                        jQuery("#pathP").html(jQuery("#pathP").html() + (roleId != 2 ? "<a onmouseover=\"this.style.cursor='hand'\" onclick=\"getCount1('" + upUserName + "','" + roleId + "','" + (++aIndex) + "'," + percent + "'" + comms + "')" + "\"> >" + upUserName + "</a>" : ""));
                    }
                    pd = 0;
                    jQuery.each(result, function (i) {
...
<th width="*" class="tab_top_m"><p id="pathP"><font class="st"> <span id="yszcs">预设占成数</span>：</font><a onmouseover="this.style.cursor='hand'" onclick="getCount1('','2','0')"><span id="H1460"> 首页</span></a></p></th>
