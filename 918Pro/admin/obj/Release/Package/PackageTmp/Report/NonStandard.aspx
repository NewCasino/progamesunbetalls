<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NonStandard.aspx.cs" ValidateRequest="false" Inherits="admin.Report.NonStandard" %>
    ...
    <script type="text/javascript">
        ...
        function SetGlobal(setLang) {
            setLang = $.SetOrGetLanguage(setLang, function () {
                languages = language;
                jQuery("#nameP").text(languages["亚洲盘及大小盘"]);
                ...
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(0)").text(languages["H1271"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(1)").text(languages["H1272"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(2)").text(languages["H1282"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(3)").text(languages["H1273"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(4)").text(languages["H1274"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(5)").text(languages["H1283"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(6)").text(languages["H1271"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(7)").text(languages["H1272"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(8)").text(languages["H1282"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(9)").text(languages["H1273"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(10)").text(languages["H1274"]);
                jQuery("#tab2>thead>tr:eq(1)").find("th:eq(11)").text(languages["H1283"]);
                switch (setLang) {
                    case "zh-cn":
        ...
        function setData() {
            ...
            var data = "language:'" + languages + "',league:'" + leagueName + "',gameId:'" + bollID + "',limi:'" + limi + "',btype:'" + btype + "',mtype:'1'";
            jQuery.AjaxCommon("/ServicesFile/ReportService/NoteSingleService.asmx/GetHdpAndOu2", data, true, false, function (json) {
                if (json.d != "none") {
                    ...
                    $.each(r, function (i) {
                        ...
                        tr.find("#HalfHmoney").text(re.homefeeh == "" ? "--" : parseInt(re.homefeeh));
                        tr.find("#HalfAmoney").text(re.awayfeeh == "" ? "--" : parseInt(re.awayfeeh));
                        tr.find("#HalfHAsum").html("" + (parseInt(re.homefeeh == "" ? "0" : re.homefeeh) + parseInt(re.awayfeeh == "" ? "0" : re.awayfeeh)));
                        tr.find("#HalfOmoney").text(re.bigfeeh == "" ? "--" : parseInt(re.bigfeeh));
                        tr.find("#HalfUmoney").text(re.smallfeeh == "" ? "--" : parseInt(re.smallfeeh));
                        tr.find("#HalfOUsum").html("" + (parseInt(re.bigfeeh == "" ? "0" : re.bigfeeh) + parseInt(re.smallfeeh == "" ? "0" : re.smallfeeh)));
                        tr.appendTo("#showInfo");
                    });
                    ...
                    jQuery("#showInfo>tr").find("#HalfHmoney").click(function () {
                        ...
                        jQuery("#delet2").dialog({ width: 1000, modal: true });
                        jQuery("#delet2").dialog({ modal: true });
                        var data = "table1:'orderdetailhdphf',table2:'orderdetailhdphfl',type:'H',game:'" + jQuery(this).parent().attr("attr") + "'";
                        repo(data, "半主");
                        div = 1;
                        Countdown1(jQuery("#timeTxt1").val(), data, "半主");
                    });
                    //半场让球and走地半场让球 客队
                    ...
                    jQuery("#showInfo>tr").find("#HalfAmoney").click(function () {
                        ...
                        jQuery("#delet2").dialog({ width: 1000, modal: true });
                        jQuery("#delet2").dialog({ modal: true });
                        var data = "table1:'orderdetailhdphf',table2:'orderdetailhdphfl',type:'A',game:'" + jQuery(this).parent().attr("attr") + "'";
                        repo(data, "半客");
                        div = 1;
                        Countdown1(jQuery("#timeTxt1").val(), data, "半客");
                    });
                    //半场大小and走地半场大小 大
                    jQuery("#showInfo>tr").find("#HalfOmoney").unbind("click");
                    jQuery("#showInfo>tr").find("#HalfOmoney").click(function () {
                        if (jQuery(this).text() == "--") return false;
                        jQuery("#delet2").dialog({ width: 1000, modal: true });
                        jQuery("#delet2").dialog({ modal: true });
                        var data = "table1:'orderdetailouhf',table2:'orderdetailouhfl',type:'O',game:'" + jQuery(this).parent().attr("attr") + "'";
                        repo(data, "半大");
                        div = 1;
                        Countdown1(jQuery("#timeTxt1").val(), data, "半大");
                    });
                    //半场大小and走地半场大小 小
                    jQuery("#showInfo>tr").find("#HalfUmoney").unbind("click");
                    jQuery("#showInfo>tr").find("#HalfUmoney").click(function () {
                        if (jQuery(this).text() == "--") return false;
                        jQuery("#delet2").dialog({ width: 1000, modal: true });
                        jQuery("#delet2").dialog({ modal: true });
                        var data = "table1:'orderdetailouhf',table2:'orderdetailouhfl',type:'U',game:'" + jQuery(this).parent().attr("attr") + "'";
                        repo(data, "半小");
                        div = 1;
                        Countdown1(jQuery("#timeTxt1").val(), data, "半小");
                    });
                }
            });
        }
                ...
                if (json.d != "none") {
                    ...
                    jQuery.each(result, function (i) {
                        ...
                        if (jQuery("#language").val() == "tw") {
                            ...
                            setLang = $.SetOrGetLanguage(setLang, function () {
                                languages = language;
                                tr.find("#teamTD").html("<span style='color:blue'>" + (type.length == 1 ? type : type.substr(1)) + "@" + result[i].Handicap + "</span>&nbsp;" + result[i].Score + "<br/>" + typ[result[i].BetType] + "<br/>" + result[i].Hometw + "&nbsp;-VS-&nbsp;" + result[i].Awaytw + "<br/>" + result[i].leaguetw + "&nbsp;&nbsp;@&nbsp;&nbsp;" + result[i].BeginTime);
                            }, "/js/IndexGlobal/");
                        }
                            ...
                            setLang = $.SetOrGetLanguage(setLang, function () {
                                languages = language;
                                tr.find("#teamTD").html("<span style='color:blue'>" + (type.length == 1 ? type : type.substr(1)) + "@" + result[i].Handicap + "</span>&nbsp;" + result[i].Score + "</span><br/>" + typ[result[i].BetType] + "<br/>" + result[i].Homecn + "&nbsp;-VS-&nbsp;" + result[i].Awaycn + "<br/>" + result[i].leaguecn + "&nbsp;&nbsp;@&nbsp;&nbsp;" + result[i].BeginTime);
                            }, "/js/IndexGlobal/");
                        }
                            ...
                            setLang = $.SetOrGetLanguage(setLang, function () {
                                languages = language;
                                tr.find("#teamTD").html("<span style='color:blue'>" + (type.length == 1 ? type : type.substr(1)) + "@" + result[i].Handicap + "</span>&nbsp;" + result[i].Score + "</span><br/>" + typ[result[i].BetType] + "<br/>" + result[i].Homeen + "&nbsp;-VS-&nbsp;" + result[i].Awayen + "<br/>" + result[i].leagueen + "&nbsp;&nbsp;@&nbsp;&nbsp;" + result[i].BeginTime);
                            }, "/js/IndexGlobal/");
                        }
                            ...
                            setLang = $.SetOrGetLanguage(setLang, function () {
                                languages = language;
                                tr.find("#teamTD").html("<span style='color:blue'>" + (type.length == 1 ? type : type.substr(1)) + "@" + result[i].Handicap + "</span>&nbsp;" + result[i].Score + "</span><br/>" + result[i].Hometh + "&nbsp;-VS-&nbsp;" + result[i].Awayth + "<br/>" + result[i].leagueth + "&nbsp;&nbsp;@&nbsp;&nbsp;" + result[i].BeginTime);
                            }, "/js/IndexGlobal/");
                        }
                            ...
                            setLang = $.SetOrGetLanguage(setLang, function () {
                                languages = language;
                                tr.find("#teamTD").html("<span style='color:blue'>" + (type.length == 1 ? type : type.substr(1)) + "@" + result[i].Handicap + "</span>&nbsp;" + result[i].Score + "</span><br/>" + result[i].Homevn + "&nbsp;-VS-&nbsp;" + result[i].Awayvn + "<br/>" + result[i].leaguevn + "&nbsp;&nbsp;@&nbsp;&nbsp;" + result[i].BeginTime);
                            }, "/js/IndexGlobal/");
                        }
        ...
        function Countdown1(time, data, type) {
            ...
            if (parseInt(time) == 0) {
                ...
                //var data = "league:'" + leagueName + "',ballteam:'" + bollID + "',language:'" + languages + "'";
                repo(data, type);
...
<th width="*" class="tab_top_m"><p id="nameP">亚洲盘&大小盘</p></th>
...
<!-- 自动刷新DIV -->
<div class="fr">
<input type="text" id="timeTxt1" value="60" class="text_01 w_30" onmouseover="this.className='text_01_h w_30'" onmouseout="this.className='text_01 w_30'" /><label id="timeUp1">5</label>&nbsp;&nbsp;&nbsp;&nbsp;
<input type="text" value="50" id="DataLength1" class="text_01 w_60" onmouseover="this.className='text_01_h w_60'" onmouseout="this.className='text_01 w_60'" /><a id="A1">条记录</a>
