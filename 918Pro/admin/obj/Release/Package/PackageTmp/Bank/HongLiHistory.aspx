<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="HongLiHistory.aspx.cs" Inherits="admin.Bank.HongLiHistory" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function SelectByWhere() {
            $("#tb2>tbody").html("");
            $.AjaxCommon("/ServicesFile/BankService/BankService.asmx/GetHistoryByWhere", "userName:'" + $.trim($("#memberSel").val()) + "',lan:'" + lan + "',name:'" + $.trim($("#memberName").val()) + "',status:'" + $.trim($("#statusSel").val()) + "',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "',type:'3',mark:'" + $("#txtmark").val() + "'", true, false, function (json) {
                if (json.d != "]") {
                    ...
                    $.each(re, function (i) {
                        ...
                        html += "<tr><td>" + (i + 1) + "</td><td><a href='javascript:void(0)' style='color:#0075a9' onclick=\"window.open('/User/UserInfo.aspx?u=" + re[i].b + "','','width=600,height=270');\">" + re[i].b + "</a></td><td style='color:red;'>" + re[i].d + "</td><td>" + re[i].e + "</td><td>" + re[i].f + "</td><td>" + "太阳城游戏游戏" + "</td><td>" + (re[i].g == "1" ? "未审核" : (re[i].g == "2" ? "成功" : "拒绝")) + "</td><td>" + re[i].mm + "</td><td>" + re[i].h + "</td></td></tr>"; //存入帐号<td>" + re[i].j + "</td>
                        html += "</tr>";
                    });
                    html += "<tr><td>总计</td><td colspan='1'></td><td style='color:red'>" + total1.toFixed(2) + "</td><td colspan='9'></td></tr>";
                    $("#tb2>tbody").html(html);
...
<th width="*" class="tab_top_m"><p id="zdjs">已审核红利</p></th>
...
<div id="main">
    ...
    <a id="jysj">提交时间</a><input type="text" id="time1WhereVal" class="inputWhere text_01 w_80" onmouseover="this.className='text_01_h w_80'" onmouseout="this.className='text_01 w_80'" readonly="readonly"  />－<input type="text" id="time2WhereVal" class="inputWhere text_01 w_100" onmouseover="this.className='text_01_h w_100'" onmouseout="this.className='text_01 w_100'" readonly="readonly"  />&nbsp;
     <a id="A1">活动</a><input type="text" id="txtmark" class="inputWhere text_01 w_80" onmouseover="this.className='text_01_h w_80'" onmouseout="this.className='text_01 w_80'" /><span style="  font-size:11px; color:Blue">(模糊查询)</span>
