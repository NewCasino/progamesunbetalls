<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="paytoaccount.aspx.cs" Inherits="admin.PTgame.paytoaccount" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>太阳城游戏平台账户转总账</title>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#depositHistory").click(function () {
                ...
                window.location.href = "FeeHistory.aspx?type=13";
            });
        ...
        function SelectByWhere() {
            $.AjaxCommon("/ServicesFile/UserService.asmx/GetPTbillnotice", "username:'" + $.trim($("#memberSel").val()) + "',type:'13',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "'", true, false, function (json) {
                if (json.d != "]") {
                    var re = jQuery.parseJSON(json.d);
                    var html = "", total1 = 0;
                    $.each(re, function (i) {
                        total1 += parseFloat(re[i].Amount);
                        html += "<tr><td>" + (i + 1) + "</td><td>太阳城游戏平台转总账</td><td><a href='javascript:void(0)'  style='text-decoration: none;color: blue' onclick=\"window.open('/Statistics/UserAnalysis.aspx?u=" + re[i].UserName + "','','width='+(window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+ ',top=0,left=0,resizable=yes,status=yes,menubar=no,scrollbars=yes');\">" + re[i].UserName + "</a></td><td class='red'>" + re[i].Amount + "</td><td>" + re[i].SubmitTime + "</td><td><a class=\"verify\" href=\"#\" attr=\"" + re[i].ID + "\">操作</a></td></tr>"; //存入帐号<td>" + re[i].j + "</td>
                    });
        ...
        function acceptBtn() {
            if (confirm("确定已从该太阳城游戏平台账户扣款？")) {
                $("#AcceptBtn").attr("disabled", true);
                $.AjaxCommon("/ServicesFile/userservice.asmx/PTtoAccount", "status:'2',ID:" + currID + "", true, false, function (json) {
                    if (json.d == "1") {
                        var re = jQuery.parseJSON(json.d);
        ...
        function refuseBtn() {
            ...
            $("#submitBtn").unbind("click").bind("click", function () {
                ...
                $.AjaxCommon("/ServicesFile/userservice.asmx/PTtoAccount", "status:'3',ID:" + currID + "", true, false, function (json) {
                    if (json.d == "1") {
                        alert("操作成功");
                        $("#verify_list").dialog("close");
                        SelectByWhere();
...
<th width="*" class="tab_top_m"><p id="zdjs">太阳城游戏平台账户转总账</p></th>
