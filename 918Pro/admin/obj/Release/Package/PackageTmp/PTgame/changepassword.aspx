<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="changepassword.aspx.cs" Inherits="admin.PTgame.changepassword" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function SelectByWhere() {
            $.AjaxCommon("/ServicesFile/UserService.asmx/GetPtthing", "username:'" + $.trim($("#memberSel").val()) + "',type:'2',status:'0',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "'", true, false, function (json) {
                if (json.d != "]") {
                    var re = jQuery.parseJSON(json.d);
                    var html = "", total1 = 0;
                    $.each(re, function (i) {
                        total1 += parseFloat(re[i].Amount);
                        html += "<tr><td>" + (i + 1) + "</td><td>" + re[i].username + "</td><td>" + re[i].password + "</td><td>" + re[i].submittime + "</td><td><a class=\"verify\" href=\"#\" attr=\"" + re[i].ID + "\">操作</a></td></tr>"; //存入帐号<td>" + re[i].j + "</td>
                    });
        ...
        function acceptBtn() {
            if (confirm("确定PT密码已修改？")) {
                ...
                $.AjaxCommon("/ServicesFile/userservice.asmx/UpdatePtthingpass", "username:'" + $("#username").html() + "',pass:'" + $("#balance").html() + "',status:'1',ID:" + currID + "", true, false, function (json) {
                    if (json.d == "1") {
                        var re = jQuery.parseJSON(json.d);
...
<th width="*" class="tab_top_m"><p id="zdjs">修改密码</p></th>
