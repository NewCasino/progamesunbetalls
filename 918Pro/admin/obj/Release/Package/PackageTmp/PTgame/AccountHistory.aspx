<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AccountHistory.aspx.cs" Inherits="admin.PTgame.AccountHistory" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function SelectByWhere() {
            $.AjaxCommon("/ServicesFile/UserService.asmx/GetPtthing", "username:'" + $.trim($("#memberSel").val()) + "',type:'" + $("#type").val() + "',status:'" + $("#status").val() + "',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "'", true, false, function (json) {
                if (json.d != "" && json.d !="]") {
                    var re = jQuery.parseJSON(json.d);
                    var html = "", total1 = 0;
                    $.each(re, function (i) {
                        total1 += parseFloat(re[i].Amount);
                        html += "<tr><td>" + (i + 1) + "</td><td>" + (re[i].type == "1" ? "申请开通" : "修改密码") + "</td><td>" + re[i].username + "</td><td>" + re[i].password + "</td><td>" + (re[i].status == "0" ? "未处理" : (re[i].status == "1" ? "已处理" : "拒绝")) + "</td><td>" + re[i].submittime + "</td><td>" + re[i].updatetime + "</td></tr>"; //存入帐号<td>" + re[i].j + "</td>
                    });
...
<th width="*" class="tab_top_m"><p id="zdjs">申请历史</p></th>
...
<input type="hidden" value="" id="timeHide" />
    ...
    <a id="A1">类型</a>&nbsp;&nbsp;<select id="type"><option value="">全部</option>
        <option value="1">申请开户</option><option value="2">修改密码</option>
    </select>&nbsp;
    <a id="zt">状态</a>&nbsp;&nbsp;<select id="status"><option value="">全部</option>
        <option value="1" selected>成功</option><option value="2">拒绝</option>
    </select>&nbsp;&nbsp;&nbsp;&nbsp;
