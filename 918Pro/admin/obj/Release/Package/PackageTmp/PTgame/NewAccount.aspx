<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewAccount.aspx.cs" Inherits="admin.PTgame.NewAccount" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>申请开通太阳城游戏平台账号</title>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#depositHistory").click(function () {
                ...
                window.location.href = "AccountHistory.aspx";
            });
        ...
        function SelectByWhere() {
            $.AjaxCommon("/ServicesFile/UserService.asmx/GetPtthing", "username:'" + $.trim($("#memberSel").val()) + "',type:'1',status:'0',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "'", true, false, function (json) {
                if (json.d != "]") {
                    var re = jQuery.parseJSON(json.d);
                    var html = "", total1 = 0;
                    $.each(re, function (i) {
                        total1 += parseFloat(re[i].Amount);
                        html += "<tr><td>" + (i + 1) + "</td><td>" + re[i].username + "</td><td>" + re[i].password + "</td><td>" + re[i].Names + "</td><td>" + re[i].submittime + "</td><td><a class=\"verify\" href=\"#\" attr=\"" + re[i].ID + "\">操作</a></td></tr>"; //存入帐号<td>" + re[i].j + "</td>
                    });
                    ...
                    $(".verify").unbind("click").bind("click", function () {
                        ...
                        $("#userName").html($tr.eq(3).html() == "" ? "--" : $tr.eq(3).html());
                        $("#htime").html($tr.eq(4).html() == "" ? "--" : $tr.eq(4).html());
        ...
        function acceptBtn() {
                if (confirm("确定太阳城游戏平台账号已开通？")) {
                    $("#AcceptBtn").attr("disabled", true);
                    $.AjaxCommon("/ServicesFile/userservice.asmx/UpdatePtthing1", "username:'" + $("#username").html() + "',pass:'" + $("#balance").html() + "',status:'1',ID:" + currID + "", true, false, function (json) {
                        if (json.d == "1") {
                            var re = jQuery.parseJSON(json.d);
        ...
        function refuseBtn() {
            ...
            $("#submitBtn").unbind("click").bind("click", function () {
                ...
                $.AjaxCommon("/ServicesFile/userservice.asmx/UpdatePtthing", "status:'2',ID:" + currID + "", true, false, function (json) {
                    if (json.d == "1") {
                        $.AjaxCommon("/ServicesFile/userservice.asmx/UpdateUserStatus", "status:'2',userName:'" + $("#username").html() + "'", true, false, function (json) {
                            if (json.d == "1") {
                                alert("操作成功");
                                $("#verify_list").dialog("close");
                                SelectByWhere();
...
<th width="*" class="tab_top_m"><p id="zdjs">申请开通太阳城游戏平台账号</p></th>
...
<div class="showdiv">
<ul>
<li><p>您确定要拒绝该申请吗？拒绝后玩家将无法进入会员中心。
</p></li>
