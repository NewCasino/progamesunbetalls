<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WinAndLose.aspx.cs" Inherits="admin.Report.WinAndLose" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
         ...
         $(function () {
             ...
             var data = "time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "',language:'" + $("#language").val() + "',status:'1',roleId:'5',ID:'0',UpUserName:'10su',mtype:'" + jQuery("#mtype").val() + "'";
             setData(data);
         });
         ...
         function getCount(name, roleIds) {
             ...
             var data = "time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "',language:'" + $("#language").val() + "',status:'1',roleId:'6',UpUserName:'" + name + "',mtype:'" + jQuery("#mtype").val() + "'";
                 upUserName = name;
                 upIds = roleId;
                 setData(data);
         ...
         function setData(data) {
             jQuery.AjaxCommon("/ServicesFile/ReportWebService.asmx/GetMatch2", data, true, false, function (json) {
                 ...
                 var TSYamount = 0;
                 if (pd) {
                     jQuery("#pathP").html(jQuery("#pathP").html() + (roleId != 2 ? "<a style=\"color:Blue;font-size:14px;\" onmouseover=\"this.style.cursor='hand'\"  >" + upUserName + "</a>" : ""));
                 }
                 pd = 0;
                 ...
                 jQuery.each(result, function (i) {
                     ...
                     if (roleId == "6" || roleId == "5") {
                         ...
                         tr.find("#UserName").html("<a style=\"color:Blue;font-size:14px;\" onmouseover=\"this.style.cursor='hand'\" " + ("onclick=\"getCount('" + result[i].UserName + "','5')") + "\">" + result[i].UserName + "</a>");
                     }
                     ...
                     //投注额
                     //tr.find("#validamount").html(0);
                     var _validamount = parseFloat(result[i].validamount);
                     tr.find("#validamount").html(_validamount);
                     totalvalidamount += _validamount;
                     SYamount = parseFloat(result[i].ckamount) - parseFloat(result[i].qkamount);
                     //输赢
                     //SYamount = parseFloat(result[i].winlose);
                     tr.find("#winandlose").html(parseFloat(SYamount).toFixed(2));
                 ...
                 tr.attr("class", "tl");
                 if (result == "") {
                     tr.html("<td height=\"20\" colspan=\"8\" style=\"background-color:#DCF0FD\">" + languages.H1413 + "</td>");
                 } else {
                     tr.html("<td height=\"20\" colspan=\"8\" style=\"background-color:#DCF0FD\"></td>");
                 }
                 tr.appendTo("#showInfo");
...
<th width="*" class="tab_top_m"><p id="pathP"><font class="st"> <span id="H1412">会员详细</span>：</font><a onmouseover="this.style.cursor='hand'" ><span id="H1460"> 首页</span></a></p></th>
