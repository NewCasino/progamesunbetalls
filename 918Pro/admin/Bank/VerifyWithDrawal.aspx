<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VerifyWithDrawal.aspx.cs" Inherits="admin.Bank.VerifyWithDrawal" %>
...
<head runat="server">
    ...
    switch (langu) {
        ...
        function SelectByWhere() {
            $.AjaxCommon("/ServicesFile/BankService/BankService.asmx/GetWithDrawalByWhere", "userName:'" + $.trim($("#memberSel").val()) + "',name:'" + $.trim($("#memberName").val()) + "',time1:'" + $("#time1WhereVal").val() + "',time2:'" + $("#time2WhereVal").val() + "'", true, false, function (json) {
                if (json.d == "-1") {
                    SysLoginOut();
                } else if (json.d != "]") {
                    ...
                    $(".verify").unbind("click").bind("click", function () {
                        ...
                        //$("#userAccount").html("<a href='javascript:void(0)' style='color:#0075a9;' onclick=\"window.open('/Statistics/UserAnalysis.aspx?u=" + $tr.eq(1).html() + "','','width='+(window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+ ',top=0,left=0,resizable=yes,status=yes,menubar=no,scrollbars=yes');\">" + $tr.eq(1).html() + "</a>");
                        $("#name").html($tr.eq(2).html() == "" ? "--" : $tr.eq(2).html());
                        $("#currency").html($tr.eq(3).html() == "" ? "--" : $tr.eq(3).html());
                        $("#money").html($tr.eq(4).html() == "" ? "--" : $tr.eq(4).html());
                        $("#userName").html($tr.eq(2).html() == "" ? "--" : $tr.eq(2).html());
                        $("#cardNo").html($tr.eq(7).html() == "" ? "--" : $tr.eq(7).html());

                        $("#type").html($tr.eq(6).html() == "" ? "--" : $tr.eq(6).html());
                        $("#province").html($tr.eq(7).attr("c") == "" ? "--" : $tr.eq(7).attr("c"));
                        $("#city").html($tr.eq(7).attr("d") == "" ? "--" : $tr.eq(7).attr("d").toString().replace("@", ""));
                        $("#branch").html($tr.eq(7).attr("e") == "" ? "--" : $tr.eq(7).attr("e"));
...
//                } else {
            if (confirm("您确定要接受该提款吗?")) {
                $("#AcceptBtn").attr("disabled", true);
                var url = "/ServicesFile/BankService/BankService.asmx/VerifyBillNotice8";
                var data = "status:'2',reason:'',id:'" + currID + "',validAmount:'" + $.trim($("#realMoney").val()) + "',bankID:" + $("#bankout").val() + ",outfee:'" + $("#outfee").val() + "',bankname:'" + $("#userName").html() + "',cardNo:'" + $("#cardNo").html() + "',type:'" + $("#type").html() + "'";
                $.AjaxCommon(url, data, true, false, function (json) {
        ...
        function refuseBtn() {
            ...
            $("#submitBtn").unbind("click").bind("click", function () {
                if (confirm("您确定要拒绝该提款吗?")) {
                    if ($.trim($("#reason").val()) == "") {
                        alert("拒绝理由不能为空");
                    } else {
                        $("#submitBtn").attr("disabled", true);
                        $.AjaxCommon("/ServicesFile/BankService/BankService.asmx/VerifyBillNotice", "status:'3',reason:'" + $("#reason").val() + "',name:'" + $("#memberName").val() + "',id:'" + currID + "',validAmount:'" + $.trim($("#realMoney").val()) + "',seef:'0'", true, false, function (json) {
                            if (json.d != "false" && json.d != "nomoney") {
                                var re = jQuery.parseJSON(json.d);
                                alert("操作成功，取消此笔取款\n当前帐户余额：" + re.a + "");
                                $("#verify_list").dialog("close");
...
<th width="*" class="tab_top_m"><p id="zdjs">提款审核</p></th>
...
<input type="hidden" id="language" value="tw"/>
    ...
    <div  style="width:95%;padding:3px;margin:2px;"><span id="VerifyWithDrawal" class="btn_04"  style=" color:Yellow; font-size:13px; font-weight:600">&nbsp;&nbsp;未审核提款&nbsp;&nbsp;</span>&nbsp;<span id="withdrawalHistory" class="btn_04" style=" font-size:11px">&nbsp;&nbsp;已审核提款&nbsp;&nbsp;</span>&nbsp;&nbsp;
...
<table width="100%"  border="0" cellpadding="1" cellspacing="1" id="verifyTb">
 ...
 <tr>
  ...
  <tr align="center">
  <td colspan="1" align="right">银行所在省：</td>
  <td colspan="3" id="province" align="left"></td>
  </tr>
  <tr align="center">
  <td colspan="1" align="right">银行所在市：</td>
  ...
  <tr>
    ...
    <tr align="center">
    <td colspan="4" style="background-color:#CDEAFC"><strong>IP信息</strong></td>
  </tr>
  <tr >
